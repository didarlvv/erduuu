...
import type { MailTypesResponse, MailTypesQueryParams, UpdateMailTypeRequest, CreateMailTypeRequest } from "@/lib/types"
import type {
...
export async function updateMailType(id: number, data: Partial<UpdateMailTypeRequest>): Promise<MailType> {
  const response = await api.patch(`/manager/mail-types/${id}`, data)
  return response.data
}

export async function createMailType(data: CreateMailTypeRequest): Promise<MailType> {
