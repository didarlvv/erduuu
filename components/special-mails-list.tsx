...
export function SpecialMailsList({ mails, isLoading }: SpecialMailsListProps) {
  ...
  const [parsedMails, setParsedMails] = useState<
    (SpecialMail & { parsedTitle: string })[]
  >([]);
        ...
        >
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              <div className="flex items-center space-x-2">
                ...
                {mail.unread_count > 0 && (
                  ...
                  >
                    {mail.unread_count} {translations[language].unread}
                  </Badge>
                )}
              </div>
              <p className="text-sm text-muted-foreground">
                {translations[language].createdBy} {mail.creator}
              </p>
            </div>
            <div className="flex items-center space-x-4">
              ...
              <Badge
                variant={
                  mail.reference === "internal" ? "default" : "secondary"
                }
              >
