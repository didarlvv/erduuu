...
export default function CreateMailPage() {
  ...
  const [availableReceivers, setAvailableReceivers] = useState<AvailableReceiver[]>([])
    ...
    async (responsibilityId: number) => {
      ...
      try {
        ...
        toast({
          ...
          description: translate("mails.create.errors.fetchReceiversError", language),
        })
      } finally {
  ...
  }, [toast, fetchAvailableReceivers, translate, language])
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <Card className="w-full max-w-md">
          <CardHeader>
            <CardTitle>{translate("mails.create.accessDenied", language)}</CardTitle>
            <CardDescription>{translate("mails.create.noPermission", language)}</CardDescription>
          </CardHeader>
        </Card>
      </div>
    )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!formData.title.trim()) {
        throw new Error(translate("mails.create.errors.titleRequired", language))
      }
      if (!formData.description.trim()) {
        throw new Error(translate("mails.create.errors.descriptionRequired", language))
      }
      if (!formData.code.trim()) {
        throw new Error(translate("mails.create.errors.codeRequired", language))
      }
      if (!formData.sender_id) {
        throw new Error(translate("mails.create.errors.senderRequired", language))
      }
      if (formData.receiver_ids.length === 0) {
        throw new Error(translate("mails.create.errors.receiverRequired", language))
      }
      if (!formData.mail_type_id) {
        throw new Error(translate("mails.create.errors.mailTypeRequired", language))
      }
      if (!date) {
        throw new Error(translate("mails.create.errors.dateRequired", language))
      }
      ...
      toast({
        ...
        description: translate("mails.create.createSuccessDescription", language),
      })
      ...
      toast({
        ...
        description:
          error instanceof Error ? error.message : translate("mails.create.createErrorDescription", language),
      })
    } finally {
      setIsLoading(false)
    }
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="sm" onClick={() => router.push("/dashboard/mails/sent")}>
            <ArrowLeft className="mr-2 h-4 w-4" />
            {translate("mails.create.backToList", language)}
          </Button>
          <h1 className="text-2xl font-semibold text-gray-800">{translate("mails.create.title", language)}</h1>
        </div>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>{translate("mails.create.createFormTitle", language)}</CardTitle>
          <CardDescription>{translate("mails.create.createFormDescription", language)}</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label className="text-sm font-medium">
                {translate("mails.create.responsibility", language)} <span className="text-red-500">*</span>
              </label>
              <Select value={formData.sender_id.toString()} onValueChange={handleResponsibilityChange}>
                <SelectTrigger>
                  <SelectValue placeholder={translate("mails.create.selectResponsibility", language)} />
                </SelectTrigger>
                <SelectContent>
                  {responsibilities.map((responsibility) => (
                    <SelectItem key={responsibility.id} value={responsibility.id.toString()}>
            ...
            {formData.sender_id !== 0 && (
              <div className="space-y-2">
                <label className="text-sm font-medium">
                  {translate("mails.create.receivers", language)} <span className="text-red-500">*</span>
                </label>
                <CustomMultiSelect
                  ...
                  placeholder={translate("mails.create.selectReceivers", language)}
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-2">
                <label htmlFor="title" className="text-sm font-medium">
                  {translate("mails.create.title", language)} <span className="text-red-500">*</span>
                </label>
                <Input
                  id="title"
                  value={formData.title}
                  onChange={(e) => setFormData((prev) => ({ ...prev, title: e.target.value }))}
                  placeholder={translate("mails.create.titlePlaceholder", language)}
                  required
                />
              </div>
              ...
              <div className="space-y-2">
                <label htmlFor="code" className="text-sm font-medium">
                  {translate("mails.create.code", language)} <span className="text-red-500">*</span>
                </label>
                <Input
                  id="code"
                  value={formData.code}
                  onChange={(e) => setFormData((prev) => ({ ...prev, code: e.target.value }))}
                  placeholder={translate("mails.create.codePlaceholder", language)}
                  required
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label htmlFor="description" className="text-sm font-medium">
                {translate("mails.create.description", language)} <span className="text-red-500">*</span>
              </label>
              <Textarea
                id="description"
                ...
                placeholder={translate("mails.create.descriptionPlaceholder", language)}
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">
                {translate("mails.create.sendDate", language)} <span className="text-red-500">*</span>
              </label>
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {date ? format(date, "PPP") : <span>{translate("mails.create.selectDate", language)}</span>}
                  </Button>
                </PopoverTrigger>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">
                {translate("mails.create.mailType", language)} <span className="text-red-500">*</span>
              </label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">{translate("mails.create.attachments", language)}</label>
              <FileUpload
            ...
            <div className="flex justify-end gap-4 pt-6">
              ...
              <Button
                type="submit"
                disabled={isLoading || isLoadingResponsibilities || isLoadingReceivers}
