...
import type { DetailedLog } from "@/lib/types";
...
export function LogsTable({
  ...
  const formatDate = (timestamp: number) => {
    return new Date(timestamp).toLocaleString(
      language === "en" ? "en-US" : language === "tk" ? "tk-TM" : "ru-RU"
    );
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Select onValueChange={onMethodFilter} defaultValue="all">
          <SelectTrigger className="w-[180px]">
            <SelectValue
              placeholder={translate("logs.filterByMethod", language)}
            />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">
              {translate("logs.allMethods", language)}
            </SelectItem>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>{translate("logs.userAgent", language)}</TableHead>
              <TableHead>{translate("logs.delay", language)}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {loading ? (
              ...
              logs.map((log) => (
                <TableRow key={log.id}>
                  ...
                  <TableCell>{`${log.user.first_name} ${log.user.last_name} (${log.user.username})`}</TableCell>
                  <TableCell>{log.ip}</TableCell>
                  <TableCell className="max-w-[200px] truncate">
                    {log.user_agent}
                  </TableCell>
                  <TableCell>{`${log.delay}ms`}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-500">
          {translate("logs.showing", language)} {startRecord}{" "}
          {translate("logs.to", language)} {endRecord}{" "}
          {translate("logs.of", language)} {total}{" "}
          {translate("logs.results", language)}
        </div>
        <div className="flex items-center space-x-2">
          <Button
            variant="outline"
          ...
          <div className="text-sm">
            {translate("logs.page", language)} {currentPage}{" "}
            {translate("logs.of", language)} {totalPages}
          </div>
          <Button
            variant="outline"
            size="sm"
